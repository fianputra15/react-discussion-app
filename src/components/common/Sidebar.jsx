...
export default function Sidebar() {
  return (
    <nav className="space-y-2 w-2/12 sm:hidden md:hidden lg:block">
      <ul className="space-y-4 fixed">
        ...
        <li className="hover:bg-red-100 hover:text-primary cursor-pointer">
          <Link data-testid="home-link" className="flex items-center gap-3 p-2" to={HOME_PAGE}>
            <MdOutlineHome color="#e64e41" width={16} />
            <span className="text-gray-400 text-sm font-medium">Home</span>
          </Link>
        </li>
        <li className="hover:bg-red-100 hover:text-primary cursor-pointer">
          <Link data-testid="leaderboard-link" className="flex items-center gap-3 p-2" to={LEADERBOARDS_PAGE}>
